...
mysql -u$user -p$pass -S /dev/shm/mysql.sock -e "use mictic;select concat(file_path,save_name) from mt_video_info where down_status in (1,2) and v_bit_rat=1 and status=1 and id in (select id from mt_video where xs_video=1);" > /opt/xuyao_baoliu.txt
if [ $? -ne 0 ];then
        echo "错误退出mysql"
        exit
fi
