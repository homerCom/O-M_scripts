...
then
	...
	# filebeat
	cd /tmp/xs
	wget -c http://219.146.255.198:8233/salt/software/filebeat-6.8.1-linux-x86_64.tar.gz
	tar -xzf filebeat-6.8.1-linux-x86_64.tar.gz -C /usr/local/xiaoshuai/
	...
	# salt
	while [ ! -f /etc/salt/minion ]
	do
		echo "saltstack start"
		wget http://219.146.255.198:8233/salt/config/repo/salt.repo -O /etc/yum.repos.d/salt.repo
		...
		pip3.6 install PyMySQL xlrd requests dmidecode netifaces docker distro
		wget http://219.146.255.198:8233/salt/config/salt/mysql.conf.j2 -O /etc/salt/minion.d/mysql.conf
	...
	# other software
	yum -y install iotop lrzsz curl unzip net-tools dmidecode dos2unix fping ftp iftop lsof man nano ncftp nmap ntfs-3g python-devel.x86_64 python-pip rsync sysstat tcpdump vim sshpass

	# hclientsh
	echo
	rsync -avL xiaoshuai@10.8.0.170::hclientsh/ /usr/local/xiaoshuai/sh/ --password-file=/etc/rsync.password
	wget -P /opt/ http://219.146.255.198:8098/share/zhang/scripts/initServer.sh
	chmod +x /opt/initServer.sh
	printf "%-60s%s\n" "xiaoshuai hclientsh" "[OK]"
	sleep 1
	
	#install python3
	#yum install -y zlib-devel bzip2-devel openssl-devel ncurses-devel sqlite-devel readline-devel tk-devel gcc make libffi-devel
	#wget -P /opt/  http://update.xshuai.com:8098/share/zhang/files/Python-3.8.11.tgz
	#cd /opt/
	...
	/usr/bin/pip3 install PyMySQL xlrd requests dmidecode netifaces docker distro
...
if [ $firstInstall -eq 1 ];
then
	echo
	echo "1.拷贝212/home/hclient 到 本机，scp -r 192.168.1.212:/home/hclient/ ."
	echo "2.chown -R tomcat:tomcat /home/hclient"
	sshpass -p 'xshotel123' scp -r root@192.168.1.212:/home/hclient /home/
	chown -R tomcat:tomcat /home/hclient
fi
service tomcat stop
sleep 2
