...
if hostname -I | grep -q 192.168.1.252;then
	dbPwd="happyview"
elif hostname -I | grep -q 192.168.1.222;then
	dbPwd="123456"
elif hostname -I | grep -q 192.168.1.223;then
	dbPwd="123456"
elif hostname -I | grep -q 192.168.1.254;then
	dbPwd="travelink"
else
	echo "dbpwd error"&&exit 1
fi
echo "数据库密码："$dbPwd

yum install dos2unix -y

mysql -uroot -p$dbPwd -e "use $htName;" > /dev/null 2>&1
[ $? -eq 0 ]&&echo "数据库已存在！"&&exit 1
echo "开始处理数据库"
mysql -uroot -p$dbPwd -e "create database $htName;"
mysql -uroot -p$dbPwd $htName < ./travelinkBag/$oldHtName.sql
