...
rm -rf $code_dir/*
/usr/bin/rsync -avz $webapp/kiosk_laundry_portal $code_dir --exclude={'*.gz','*.zip','*.log','*.sql'}
...
cd /back/rsync/code
cpulimit -l 50 zip -r web_code_$date_minute.zip *
...
/usr/bin/rsync -avz `find $webapp/kiosk_laundry_portal/application/logs/ -type f -name "*.log" -mmin +60 -o -name "*.log.gz"` $logs_dir/laundry_portal/
/usr/bin/rsync -avz `find $webapp/kiosk_value_code/application/logs/ -type f -name "*.log" -mmin +60 -o -name "*.log.gz"` $logs_dir/value_code/
/usr/bin/rsync -avz `find $serverapp/TTI_ReportServer/logs -type f -name "*.log.*" -mmin +60` $logs_dir/reportServer/
/usr/bin/rsync -avz `find /tmp/TokenServerLog/* -type f -mmin +60` $logs_dir/tokenServer/
/usr/bin/rsync -avz `find /var/www/policyServerLog/* -type f -mmin +60` $logs_dir/policyServer/

line=`ls $serverapp/ |grep tcp|wc -l`
dir=`ls $serverapp/ |grep tcp`
if [ $line -eq "1" ] ;then
    /usr/bin/rsync -avz `find $serverapp/$dir/logs/ -name "*.log.*" -mmin +60` $logs_dir/tcp/
fi
...
if [ -d $serverapp/TTI_Proxy ] ;then
	...
	/usr/bin/rsync -avz `find $serverapp/TTI_Proxy/logs -name "*.log.*" -mmin +60` $logs_dir/proxy/	
fi

#gzip logs
cpulimit -l 50 find $logs_dir/ -type f -exec gzip --rsyncable {} \;
